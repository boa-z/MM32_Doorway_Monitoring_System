...
使用`GuestAPI` 的 GET 方法，直接 `localhost:5000/guest` 即可获取所有访客的信息，输入`localhost:5000/guest/1` 即可获取 id 为 1 的访客的信息。
...
使用`UserAPI` 的 GET 方法，直接 `localhost:5000/user` 即可获取所有用户的信息，输入`localhost:5000/user/1` 即可获取 id 为 1 的用户的信息。
...
使用`AlarmAPI` 的 GET 方法，直接 `localhost:5000/alarm` 即可获取所有警报的信息，输入`localhost:5000/alarm/1` 即可获取 id 为 1 的警报的信息，输入`localhost:5000/alarm/1/file` 即可获取 id 为 1 的警报的图片。
使用`AlarmAPI` 的 POST 方法，直接上传图片文件，支持 jpg/jpeg/png，即可以创建新警报。例如，使用 curl 命令：`curl -F "image=@1.png" http://localhost:5000/alarms` 即可上传 1.jpg 文件，创建新警报。`alarm_time` 会自动填充为当前时间。PUT 和 DELETE 方法的使用方法类似，是针对前端编写的，同时部分功能缺失，感觉也没有使用的必要，建议不要使用。
...
使用`GuestAPI` 的 GET 方法，直接 `serverip:5000/guest` 即可获取所有访客的信息，输入`localhost:5000/guest/1` 即可获取 id 为 1 的访客的信息。
使用`GuestAPI` 的 POST 方法，数据格式如下
...
使用`UserAPI` 的 GET 方法，直接 `serverip:5000/users` 即可获取所有用户的信息，输入`localhost:5000/user/1` 即可获取 id 为 1 的用户的信息。
...
使用`AlarmAPI` 的 GET 方法，直接 `serverip:5000/alarms` 即可获取所有警报的信息，输入`localhost:5000/alarm/1` 即可获取 id 为 1 的警报的信息。输入`localhost:5000/alarm/1/file` 即可获取 id 为 1 的警报的图片。
